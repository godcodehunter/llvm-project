...
public:
  ...
  [[deprecated("Instead use overload with Caller")]] explicit Module(
      StringRef ModuleID, LLVMContext &C);

  explicit Module(const Caller *Caller, StringRef ModuleID, LLVMContext &C);
  ...
  [[deprecated("Instead use overload with Caller")]] 
  void setModuleIdentifier(StringRef ID) {
    setModuleIdentifier(nullptr, ID);
  }

  void setModuleIdentifier(const Caller *Caller, StringRef ID) {
    ModuleID = std::string(ID);
    GLOBAL_OBSERVER.updateModule(this, Caller, UpdateID{ModuleID});
  }

  /// Set the module's original source file name.
  void setSourceFileName(StringRef Name) { 
    setSourceFileName(nullptr, Name);
  }

   [[deprecated("Instead use overload with Caller")]] 
  void setSourceFileName(const Caller *Caller, StringRef Name) { 
    SourceFileName = std::string(Name); 
    GLOBAL_OBSERVER.updateModule(this, Caller, UpdateSourceFile{SourceFileName});
  }
  ...
  void setModuleInlineAsm(StringRef Asm) {
    setModuleInlineAsm(nullptr, Asm);
  }

  void setModuleInlineAsm(const Caller *Caller, StringRef Asm) {
    GlobalScopeAsm = std::string(Asm);
    GLOBAL_OBSERVER.updateModule(this, Caller, UpdateSetGlobalAsm{SourceFileName});
  ...
  [[deprecated("Instead use overload with Caller")]] 
   void appendModuleInlineAsm(StringRef Asm) {
    GLOBAL_OBSERVER.updateModule(this, Caller, UpdateAppendGlobalAsm{std::string(Asm)});
    GlobalScopeAsm += Asm;
    if (!GlobalScopeAsm.empty() && GlobalScopeAsm.back() != '\n')
      GlobalScopeAsm += '\n';
  }
  ...
  void appendModuleInlineAsm(const Caller *Caller, StringRef Asm) {
    GLOBAL_OBSERVER.updateModule(this, Caller, UpdateAppendGlobalAsm{std::string(Asm)});
    GlobalScopeAsm += Asm;
    if (!GlobalScopeAsm.empty() && GlobalScopeAsm.back() != '\n')
      GlobalScopeAsm += '\n';
